...
POSSIBLE_TASKS = ["train", "test", "match", "train-test", "train-match", "debug", "smatch"]
...
def read_edgelist(edgelist_path):
    '''
    read file with edges' information
    :param edgelist_path: path to file .egdelist
    :return node_types: prefixes, type: list, example: ["3#__tn", "3$__tt", "5$__idx", "1$__cid"]
    :return edgelist: edges and weights, type: list, example: [A,B,0.5]
    '''
    with open(edgelist_path, "r") as fp:
        ...
        for idx, line in enumerate(fp):
            # print("idx: ", idx)
            if idx == 0:
                ##### the first line is prefines (aka node type)
                node_types = next(csv.reader(StringIO(line), skipinitialspace=True)) #### skipinitialspace=True: Skip spaces after commas/跳过逗号后的空格
                print(node_types)
                # node_types = line.strip().split(",")
            else:
                l = next(csv.reader(StringIO(line), skipinitialspace=True))
                # print(l)
                # l = line.strip().split(",")
                l1 = l[:2] ##### get two first values which are nodes
                if len(l) > 2: ##### if there's weight
                    for _ in l[2:]: ##### for values from the third one
                        w1 = float(_)
                        l1.append(w1)  ##### add weight to list
                edgelist.append(l1)
...
def check_config_validity(config):
    ...
    #### Path checks
    if "train" in config["task"] and not os.path.exists(config["edgelists_file"]):
        raise IOError("Input file {} not found.".format(config["edgelists_file"]))
