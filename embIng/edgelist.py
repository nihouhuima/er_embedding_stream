...
class EdgeList:
    ...
    def draw(self, value, flatten, intersection, frequencies, rid, col, numeric_columns, fp):
        ...
        if cell_value:
            # Tokenize cell_value depending on the chosen strategy. 分词
...
if __name__ == "__main__":
    ...
    ###### batch, read dataset file, get dataframe
    df = pd.read_csv(dfpath, low_memory=False, quotechar='"', encoding="utf8", on_bad_lines='warn')


    pref = ["3#__tn", "3$__tt", "5$__idx", "1$__cid"]

    
    el = EdgeList(df, edgefile, pref, info_file, flatten=False)
