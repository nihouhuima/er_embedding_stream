...
def generate_walks(configuration, graph, intersection=None):
    sentences = []
    # n_sentences = int(float(configuration["n_sentences"]))
    strategies = configuration["walks_strategy"]
    sentence_length = int(configuration["sentence_length"])
    backtrack = configuration["backtrack"]

    if intersection is None:
        intersection = set(graph.delta_cell_lists)
        # print("intersection", intersection)
        graph.delta_cell_lists = []
        graph.delta_nodes = {}
    ...
    if random_walks_per_node > 0:
        pbar = tqdm(desc="# Sentence generation progress: ", total=len(intersection)*configuration['random_walks_per_node'])
        for cell in intersection:
            # if cell in intersection:
            ######## random walk for each node
            ###### wormhole
            # print('random walk for node: ', cell)
            r = []
            ...
            if configuration["write_walks"]:
                if len(r) > 0:
                    ...
                    pass
            sentences += r
            # print("sentences test: ", sentences)
...
def dynrandom_walks_generation(configuration, graph):
    ...
    # Generating walks.
    ###### we are in this situation, no intersection
