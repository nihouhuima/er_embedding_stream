    ...
    def add_neighbor(self, neighbor, weight):
        if neighbor not in self.neighbors:
            ...
            if neighbor.node_class['isfirst']:
                self.startfrom = np.append(self.startfrom, neighbor.name)
        else:
...
class DynGraph:
    ...
    def add_edge(self, node_from, node_to, weight_forward, weight_back=None):
        ...
        if l2 > l1:
            ...
            if weight_back is not None:
                ...
                node_to.add_neighbor(node_from, weight_forward)
    ...
    def update_graph(self, edgelist,sim_list=None, delta=True):
        ...
        for line in tqdm(edgelist, desc='# Loading edge value. '):
            ...
            for _n in [n1, n2]:
                ...
                if node_prefix == "idx":
                    num_id = node_name.split('__', 1)[1]
                    if float(num_id)>float(self.num_ids):
                        self.num_ids=num_id
                    ...
                    for idx, val in enumerate(valsplit):
                        ...
                        if nn not in self.nodes:
                            node = Node(nn, node_prefix, node_class=self.node_classes[node_prefix],
                                        numeric=False)
                            self.nodes[nn] = node
                        if delta and (nn not in self.delta_nodes):
                            self.delta_nodes[node_name] = self.nodes[nn]
                ...
                ###### create node if node value is not in graph
                ...
                if delta and (node_name not in self.delta_nodes):
                        self.delta_nodes[node_name] = self.nodes[node_name]
            ...
            for _1 in to_link[0]:
                for _2 in to_link[1]:
                    if _1 != _2:
                        # print(f"create edge for {_1.name} and {_2.name}")
                        self.add_edge(_1, _2, w1, w2)

        ##### Check its neighbors and normalize the weights of the neighbors
        to_delete = []
        if len(self.nodes) == 0:
        ...
        if delta:
            ...
            it = self.nodes
        for node_name in tqdm(it, desc='# Preparing aliased randomizer.'):
            # print(node_name)
            if it[node_name].node_class['isroot']:
                if node_name not in self.cell_list:
                    self.cell_list.append(node_name)
                if delta and (node_name not in self.delta_cell_lists):
                #     print("add delta")
                    self.delta_cell_lists.append(node_name)
            # print("cell list: ", node_name)
    ...
    def __init__(self, edgelist, prefixes, sim_list=None, flatten=[]):
        ...
        self.cell_list = []
        self.delta_cell_lists = []
        self.delta_nodes = {}
        self.possible_first = []
        self.num_ids = 0 # the number of records
        ...
        if flatten == 'all':
            print('# Tokenization: flatten = all, all strings will be expanded.')
            self.to_flatten = self.node_classes
        elif len(self.to_flatten) > 0:
            print('# Tokenization: Expanding columns: [{}].'.format(', '.join(self.to_flatten)))
        elif not flatten:
            print('# Tokenization: All values will be tokenized. ')
        else:
            print('# Tokenization: All values will be tokenized. ')

        if edgelist != []:
            print("# Adding data from initial edgelist...")
            self.update_graph(edgelist, sim_list=sim_list, delta=False)
        else:
...
def dyn_graph_generation(configuration, edgelist, prefixes):
    """
    ...
    g = DynGraph(edgelist=edgelist, prefixes=prefixes, sim_list=list_sim, flatten=flatten)
