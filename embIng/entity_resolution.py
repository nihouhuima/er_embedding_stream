...
###### A appears in candidates'list of B, and B appears in candidates'list of A
...
def build_similarity_structure(model_file, viable_lines, n_items, strategy,
    ...
    if strategy == 'basic':
        ...
        for n in tqdm(nodes, desc='# ER - Finding node matches: '):
            ####### n_top: n_closest_neighbors_to_study
            ms = model.most_similar(str(n), topn=n_top)
            ####### example of ms : [('queen', 0.7543209), ('prince', 0.6765987), ('monarch', 0.6453021)]
            ####### !!!!!!!! HERE you have SIMILARITY DEGREE !!!!!!!!!
    ...
    ###### structure of most_similar: {node1: [cand1, cand2, ...], node2: [cand1, cand2, ...]}
    return most_similar
...
def entity_resolution(input_file: str, configuration: dict,
    ...
    # model_file: embedding file only with idx, viable_lines : all id numbers, aka.idx
    model_file, viable_lines = _prepare_tests(input_file)
    with open(info_file, 'r', encoding='utf-8') as fp:
        line = fp.readline()
        ######## n_items: number of records in dataset1
